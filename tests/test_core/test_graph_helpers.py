"""
Tests for the GraphDB write helpers (upsert_node, ensure_edge,
purge_file_data) on the DuckDB backend.

Each test asserts observable state after a helper call, pinning the
contract the indexer relies on.
"""
...
def _count(db, table_duckdb: str) -> int:
    """Row count for a node table."""
    result = db.execute(f"SELECT count(*) AS c FROM {table_duckdb}")
    return int(result.get_next()[0])


def _edge_count(db, table_duckdb: str) -> int:
    result = db.execute(f"SELECT count(*) AS c FROM {table_duckdb}")
    return int(result.get_next()[0])
...
class TestUpsertNode:
    ...
    def test_upsert_updates_props(self, graphdb):
        graphdb.upsert_node("File", "path", "/a.py", {"lang": "python", "mtime": 1.0})
        graphdb.upsert_node(
            "File", "path", "/a.py", {"lang": "typescript", "mtime": 2.0}
        )
        # Query the lang field back.
        r = graphdb.execute(
            "SELECT lang AS l, mtime AS m FROM file WHERE path = '/a.py'"
        )
        row = r.get_next()
...
class TestEnsureEdge:
    def test_creates_calls_edge(self, graphdb):
        ...
        assert _edge_count(graphdb, "edge_calls") == 1
    ...
    def test_imports_edge_with_symbol(self, graphdb):
        ...
        assert _edge_count(graphdb, "edge_imports") == 1
...
class TestFindNodes:
    ...
    def test_contains_substring(self, graphdb):
        graphdb.upsert_node(
            "Function", "id", "a::handle_x", {"name": "handle_x", "file_path": "/a.py"}
        )
        graphdb.upsert_node(
            "Function", "id", "a::handle_y", {"name": "handle_y", "file_path": "/a.py"}
        )
        graphdb.upsert_node(
            "Function", "id", "a::other", {"name": "other", "file_path": "/a.py"}
        )
        rows = graphdb.find_nodes(
...
class TestFindNodesOrderBy:
    def test_order_by_field(self, graphdb):
        graphdb.upsert_node(
            ...
            "id",
            "a::z",
            {"name": "z", "file_path": "/a.py", "start_line": 30},
        )
        graphdb.upsert_node(
            "Function",
            "id",
            "a::a",
            {"name": "a", "file_path": "/a.py", "start_line": 10},
        )
        graphdb.upsert_node(
            "Function",
            "id",
            "a::m",
            {"name": "m", "file_path": "/a.py", "start_line": 20},
        )
        rows = graphdb.find_nodes(
...
class TestFindNodesWithoutIncoming:
    def test_function_with_no_callers_returned(self, graphdb):
        ...
        graphdb.upsert_node(
            ...
            "id",
            "a::bar",
            {"name": "bar", "file_path": "/a.py", "start_line": 4, "end_line": 5},
        )
        graphdb.upsert_node(
            "Function",
            "id",
            "a::baz",
            {"name": "baz", "file_path": "/a.py", "start_line": 8, "end_line": 9},
        )
    ...
    def test_exclude_underscore_prefix(self, graphdb):
        graphdb.upsert_node(
            ...
            "id",
            "a::_priv",
            {"name": "_priv", "file_path": "/a.py", "start_line": 1, "end_line": 2},
        )
        graphdb.upsert_node(
            "Function",
            "id",
            "a::pub",
            {"name": "pub", "file_path": "/a.py", "start_line": 4, "end_line": 5},
        )
        rows = graphdb.find_nodes_without_incoming(
...
class TestFindNeighbors:
    def setup_data(self, db):
        db.upsert_node(
            "Function", "id", "a::caller", {"name": "caller", "file_path": "/a.py"}
        )
        db.upsert_node(
            "Function", "id", "a::callee", {"name": "callee", "file_path": "/a.py"}
        )
        db.upsert_node(
            "Function", "id", "a::other", {"name": "callee", "file_path": "/b.py"}
        )
...
class TestReachViaEdge:
    def test_single_hop(self, graphdb):
        ...
        rows = graphdb.reach_via_edge(
            "IMPORTS", "/a.py", max_depth=1, return_fields=["path"]
        )
        assert len(rows) == 1
    ...
    def test_two_hop(self, graphdb):
        ...
        deep = graphdb.reach_via_edge(
            "IMPORTS", "/a.py", max_depth=2, return_fields=["path"]
        )
        paths = {r["path"] for r in deep}
        assert paths == {"/b.py", "/c.py"}

        shallow = graphdb.reach_via_edge(
            "IMPORTS", "/a.py", max_depth=1, return_fields=["path"]
        )
...
class TestPurgeFileData:
    def test_purge_drops_functions_and_edges(self, graphdb):
        ...
        assert _count(graphdb, "function") == 0
        assert _edge_count(graphdb, "edge_calls") == 0
    ...
    def test_purge_drops_imports_outgoing(self, graphdb):
        ...
        graphdb.purge_file_data("/a.py")
        assert _edge_count(graphdb, "edge_imports") == 0
    ...
    def test_purge_other_files_untouched(self, graphdb):
        ...
        graphdb.purge_file_data("/a.py")
        # b.py's data intact
        assert _count(graphdb, "function") == 1
